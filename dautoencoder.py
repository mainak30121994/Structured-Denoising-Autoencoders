...
#generating alpha
N = 784
alpha = np.random.random_integers(0,1,size=(N))
alpha_s=tf.constant(alpha,dtype="float32")
...
def vis(images, save_name):
    ...
    for g,count in zip(gs,range(int(dim))):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
    plt.savefig(save_name + '_vis1.png')
...
def custom_cost(X,mask,Z):
    tilde_X=mask*X
    
    error=tf.multiply(1.0,Z-tilde_X)+tf.multiply(alpha_s,tilde_X-X)    

   
    # return tf.reduce_sum(tf.pow(X-Z, 2))#this is for denoising encoder
    return tf.reduce_sum(tf.pow(error, 2))


# create cost function
cost = custom_cost(X,mask,Z) # minimize squared error
